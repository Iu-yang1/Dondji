...
| CW | `MODE → CW` / `电报` | DSB 同源的零中频接收；PTT 作为直键，发射无调制载波，仍走 PTT/TOT/BCL/功率保护；进入 CW 时默认 U6 | 已实现，频谱/功率待测 |
| CW 键控器 | CW 模式 `F+8` | 独立、非阻塞的国际 Morse A-Z/0-9/空格键控器；`MENU` 切换 WPM/内容，`▲/▼` 设置 5–30 WPM，数字键多击输入英文，PTT 发射 | 已实现；需用假负载测量点划时序、功率和杂散 |
...
| `0x062000–0x062FFF` | **CW 键控器 v1 配置区**：带 CRC 的 WPM（5–30） | 新增；未知版本拒绝覆盖 |
...
- ELF：text 119808 B、data 552 B、bss 13568 B；链接器 FLASH 120360 B、RAM 14120 B。
- `Dondji.cn-rf.bin`：120360 B；上限 120832 B；余量 472 B。
